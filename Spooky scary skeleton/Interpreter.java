...
public class Interpreter {
	private Board board;
	Interpreter(Board given, Controller c){
		board = given;
	}
	void getCommands() {
		...
		while(!cmdParts[0].equals("exit") && !cmdParts[0].equals("file")) {
			switch(cmdParts[0]) {
			case "tile": handleTile(cmdParts); break;
			case "btile": handleBTile(cmdParts); break;
			case "orangutan": handleOrangutan(cmdParts); break;
			case "panda": handlePanda(cmdParts); break;
			case "tpanda": handleTPanda(cmdParts); break;
			case "spanda": handleSPanda(cmdParts); break;
			case "jpanda": handleJPanda(cmdParts); break;
			case "gamemachine": handleGM(cmdParts); break;
			case "chocolatemachine": handleCM(cmdParts); break;
	...
	void handleBTile(String[] cmd) {
		...
		if (t == null) {
			t = new BreakableTile(Integer.parseInt(cmd[1]));
			t.name = cmd[2];
			board.addTile(t);
		}
		t.removeNeighbours();
	...
	void handleOrangutan(String[] cmd) {
	...
	void handleXD(String[] cmd) {
		if(elementCheck(cmd)) return;
		ExitDoor gm = new ExitDoor();
		gm.setName(cmd[1]);
		Tile t = board.getTByName(cmd[2]);
		t.setElement(gm);
		Controller.getElements().add(gm);
	}
	void handleWD(String[] cmd) {
		if(cmd.length<2) {
	...
	/*Checker functions*/
	public boolean orangutanCheck(String[] cmd) {
		if(cmd.length<3) {
	...
	public boolean pandaCheck(String[] cmd) {
		if(cmd.length<3) {
			Indent.print("Missing parameters.");
			return true;
		}
	...
	public boolean steppableCheck(String[] cmd) {
		if(cmd.length<3) {
			Indent.print("Missing parameters.");
			return true;
		}
	...
	public boolean elementCheck(String[] cmd) {
		if(cmd.length<3) {
			Indent.print("Missing parameters.");
			return true;
		}
	...
	public boolean wardrobeCheck(String[] cmd) {
		if(cmd.length<3) {
			Indent.print("Missing parameters.");
			return true;
		}
