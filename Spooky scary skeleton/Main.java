	...
	public static void mainLoop() throws Exception {
		...
		switch (answer) {
			case "O":
				...
				Controller.addOrangutan(o);
				jatekos = o;
				break;
			case "P":
				Panda p = new Panda();
				Controller.addPanda(p);
				jatekos = p;
				break;
			case "TP":
				TiredPanda tp = new TiredPanda();
				Controller.addPanda(tp);
				jatekos = tp;
				break;
			case "SP":
				ScaredPanda sp = new ScaredPanda();
				Controller.addPanda(sp);
				jatekos = sp;
				break;
			case "JP":
				JumpingPanda jp = new JumpingPanda();
				Controller.addPanda(jp);
				jatekos = jp;
				break;
			default:
				Orangutan ok = new Orangutan();
				Controller.addOrangutan(ok);
				jatekos = ok;
		}
		...
		if (life == -1) {
			switch (answer) {
				case "JT":
					position = new Tile(true, false, null);
					board.addTile(position);
					break;
				case "WT":
					position = new Tile(false, true, null);
					board.addTile(position);
					break;
				case "TT":
					position = new Tile(false, false, new Armchair());
					board.addTile(position);
					break;
				case "N":
					position = new Tile();
			...
			switch (answer) {
				case "JT":
					position = new BreakableTile(true, false, null, life);
					board.addTile(position);
					break;
				case "WT":
					position = new BreakableTile(false, true, null, life);
					board.addTile(position);
					break;
				case "TT":
					position = new BreakableTile(false, false, new Armchair(), life);
					board.addTile(position);
					break;
				case "N":
					position = new BreakableTile(life);
					board.addTile(position);
					break;
				default:
					position = new BreakableTile(life);
